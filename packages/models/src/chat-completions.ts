...
export interface ChatCompletionsConfig {
  /** Receipt label for the provider, e.g. "deepseek" or "openrouter". */
  provider: string;
  /** Fixed https endpoint chosen by the deployment wrapper, never by a task. */
  endpoint: string;
  apiKey: string;
  model: string;
  timeoutMs?: number;
  fetch?: typeof globalThis.fetch;
  /** Thrown verbatim when the configuration is invalid. Must not contain values. */
  invalidConfigMessage: string;
  /** Provider-specific request fields merged after the standard ones. */
...
/**
 * OpenAI-compatible chat-completions port: bounded request and response, validated
 * provenance, sanitised failures. Extracted unchanged from the DeepSeek adapter so a
 * second provider reuses the same hardened code path instead of a copy.
 */
export function createChatCompletionsPort(config: ChatCompletionsConfig): ModelPort {
  if (
    typeof config.apiKey !== "string" ||
    !/^[^\s]+$/.test(config.apiKey) ||
    !Metadata.safeParse(config.model).success
  )
    throw new Error(config.invalidConfigMessage);
  ...
  const { apiKey, model, provider, endpoint } = config;
  ...
  return {
    async generate(raw, options = {}) {
      ...
      const base = {
        ...
        provider,
        model,
        requestDigest: modelDigest({ provider, model, request }),
      };
      const finish = () => ({
        ...base,
