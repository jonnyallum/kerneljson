...
- **Known code-quality gaps (flagged, non-blocking):** `run_e2e_proof()` asserts a `repository.read` result `execute_task` may not produce. **RESOLVED (this session, correction):** `install_scheduled_task()` no longer wires probe-only `--check` — read directly from `scripts/spawner-watchdog.py` line 232, the `schtasks` command it builds has no `--check` flag, so the installed task runs the default restarting `watchdog_cycle`. Earlier drafts of this doc described this as fixed "operationally" (a corrected manual `schtasks` invocation); that undersold it — the fix is in the script itself, so a fresh `--install-task` run now produces the correct task without anyone needing to hand-correct the command.
